...
glm::vec3 cart2sph(const glm::vec3& cart) {
	return glm::vec3(
			...
			atan2(cart.y,cart.x),
			atan2(hypot(cart.y,cart.x),cart.z));
}
...
void draw_prism(unsigned sides, double radius, double height)
{
	...
	glBegin(GL_QUADS);
		for(unsigned s=0; s<sides;++s){
			...
			auto t1=glm::vec2(0.75 / sides *(s + 0),0.0f);
			auto t2=glm::vec2(0.75 / sides *(s + 1),0.0f);
			auto t3=glm::vec2(0.75 / sides *(s + 1),1.0f);
			auto t4=glm::vec2(0.75 / sides *(s + 0),1.0f);
...
void draw_pyramid(unsigned sides, double radius, double height)
{
	...
	for(unsigned s=0; s<sides;++s){
		...
		glm::vec3 vertex_cyl{radius, phi, 0.f};
		auto vertex_cart = cyl2cart(vertex_cyl);
		auto t=glm::vec2(0.75f + 0.25f*sin(phi),
						 0.5f - 0.5f*cos(phi));
		glTexCoord2fv(&t[0]);
		glVertex3fv(&vertex_cart[0]);
	}
	...
	glBegin(GL_TRIANGLES);
		for(unsigned s=0; s<sides;++s){
			...
			float phi2 = delta_phi * (s + 1);
			float phi_n = (phi1+phi2)/2.f;
			glm::vec3 normal_cph {1.f,phi_n,theta_n};
			auto normal_cart = sph2cart(normal_cph);
			auto v1=cyl2cart(glm::vec3 (radius,phi1,0.f));
			auto v2=cyl2cart(glm::vec3 (radius,phi2,0.f));
			auto tt=glm::vec2(0.25f,
							  0.5f );
			auto t1=glm::vec2(0.25f + 0.25f*sin(phi1),
							  0.5f + 0.5f*cos(phi1));
			auto t2=glm::vec2(0.25f + 0.25f*sin(phi2),
							  0.5f + 0.5f*cos(phi2));
	...
	float delta_theta = Pi / rows;
		...
		for (unsigned row = 0; row < rows; ++row){
			for (unsigned col = 0; col < cols; ++col){
				...
				auto t1 = glm::vec2(1.f * (col+0) /cols,
									1.f * (row+0) /rows);
				auto t2 = glm::vec2(1.f * (col+1) /cols,
									1.f * (row+0) /rows);
				auto t3 = glm::vec2(1.f * (col+1) /cols,
									1.f * (row+1) /rows);
				auto t4 = glm::vec2(1.f * (col+0) /cols,
									1.f * (row+1) /rows);
				glNormal3fv(&n[0]);
				glTexCoord2fv(&t1[0]); glVertex3fv(&v1[0]);
				glTexCoord2fv(&t2[0]); glVertex3fv(&v2[0]);
